...
const EMPTY_ITEM = { name: '', description: '', price: '', available: true, image: null }

function ItemRow({ catId, item, onUpdate, onDelete }) {
  const [editing, setEditing] = useState(false)
  const [form, setForm] = useState({ ...item, price: item.price.toString(), image: item.image || null })

  const save = () => {
    onUpdate(catId, item.id, { ...form, price: parseFloat(form.price) || 0 })
    setEditing(false)
  }
  ...
  return (
    <div className="flex items-center justify-between p-3 rounded-xl transition-all hover:bg-[#F9FAFB] group"
      ...
      <div className="flex items-center gap-3 flex-1 min-w-0">
        ...
        <div className="min-w-0">
          <div className="flex items-center gap-2">
            ...
            {!item.available && (
              <span className="text-xs px-2 py-0.5 rounded-full" style={{ backgroundColor: '#FEE2E2', color: '#DC2626' }}>Indispo</span>
            )}
          </div>
      ...
      <div className="flex items-center gap-2 flex-shrink-0 ml-2">
        <span className="font-bold text-sm" style={{ color: '#D97706' }}>{parseFloat(item.price).toFixed(2)}€</span>
        <button onClick={() => setEditing(true)}
          className="p-1.5 rounded-lg opacity-0 group-hover:opacity-100 transition-all"
          style={{ color: '#6B7280', backgroundColor: '#F3F4F6' }}>
          <Edit2 size={14} />
        </button>
        <button onClick={() => { if (confirm(`Supprimer "${item.name}" ?`)) onDelete(catId, item.id) }}
          className="p-1.5 rounded-lg opacity-0 group-hover:opacity-100 transition-all"
...
function AddItemForm({ catId, onAdd, onCancel }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="p-4 rounded-xl space-y-3 mt-2"
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          ...
          <input type="text" required value={form.name}
            ...
            className="w-full px-3 py-2 rounded-lg border text-sm outline-none"
            style={{ borderColor: '#E5E7EB', color: '#1C1917' }}
            placeholder="Nom du plat" autoFocus />
        </div>
        <div>
      ...
      <div>
        <label className="text-xs font-medium block mb-1" style={{ color: '#14532D' }}>Image (optionnel)</label>
        <ImagePicker value={form.image} onChange={v => setForm(p => ({ ...p, image: v }))} />
      </div>
      ...
      <div className="flex items-center justify-between px-5 py-4"
        ...
        {editingName ? (
          <div className="flex items-center gap-2 flex-1">
            <input type="text" value={nameForm.icon}
            ...
            <input type="text" value={nameForm.name}
              ...
              style={{ borderColor: '#E5E7EB', color: '#1C1917' }} autoFocus />
            <button onClick={saveName} className="p-1.5 rounded-lg text-white" style={{ backgroundColor: '#D97706' }}>
              <Check size={14} />
            </button>
            <button onClick={() => setEditingName(false)} className="p-1.5 rounded-lg" style={{ color: '#9CA3AF' }}>
              <X size={14} />
            </button>
          </div>
        ) : (
        ...
        {!editingName && (
          <div className="flex items-center gap-1">
            ...
            <button onClick={() => { if (confirm(`Supprimer la catégorie "${cat.name}" et tous ses articles ?`)) onDeleteCategory(cat.id) }}
              className="p-1.5 rounded-lg transition-all hover:opacity-80"
              style={{ color: '#DC2626', backgroundColor: '#FEE2E2' }}>
              <Trash2 size={14} />
            </button>
      ...
      {!collapsed && (
        <div className="p-4">
          ...
          <div className="space-y-1 mb-3">
            {cat.items.map(item => (
              <ItemRow key={item.id} catId={cat.id} item={item} onUpdate={onUpdateItem} onDelete={onDeleteItem} />
            ))}
          </div>
          {addingItem ? (
            <AddItemForm catId={cat.id} onAdd={onAddItem} onCancel={() => setAddingItem(false)} />
          ) : (
      ...
      {showAddCat && (
        <form onSubmit={handleAddCategory} className="p-5 rounded-2xl space-y-3"
          ...
          <div className="flex gap-3">
            ...
            <input type="text" required value={newCat.name} onChange={e => setNewCat(p => ({ ...p, name: e.target.value }))}
